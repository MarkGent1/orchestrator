...
class AdoMcpClient:
    ...
    def _drain_stderr(self):
        try:
            for line in iter(self.proc.stderr.readline, b""):
                if line:
                    print(f"[ADO MCP stderr] {line.decode('utf-8', errors='replace').rstrip()}")
        except Exception:
            pass
    ...
    async def _call(self, method: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.proc.poll() is not None:
            raise RuntimeError(
                f"ADO MCP server process has already exited (code {self.proc.returncode})"
            )
        ...
        while True:
            if self.proc.poll() is not None:
                raise RuntimeError(
                    f"ADO MCP server exited unexpectedly (code {self.proc.returncode}) "
            ...
            try:
                ...
                raise RuntimeError(
                    f"Timed out after {self.timeout}s waiting for ADO MCP response to '{method}'"
                )

            if not line:
                code = self.proc.poll()
                raise RuntimeError(
                    f"ADO MCP server closed its stdout unexpectedly (exit code {code}) "
                    f"while waiting for a response to '{method}'"
                )
